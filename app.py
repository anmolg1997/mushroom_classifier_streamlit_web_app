...
def main():
    ...
    st.markdown("Are your Mushrooms Edible/Poisonous ? 🍄")
    st.sidebar.markdown("Are your Mushrooms Edible/Poisonous ? 🍄")
    ...
    if classifier=="Logistic Regression":
        ...
        max_iter = st.sidebar.slider("Max Iteration", 100, 2000, step=1, key='max_iter')
    ...
    if classifier=="Random Forest":
        ...
        if st.sidebar.button("Classify", key='classify'):
            st.subheader(classifier)
            model = RandomForestClassifier(n_estimators=n_estimators, bootstrap=bootstrap, max_depth=max_depth)
            model.fit(x_train, y_train)
            accuracy = model.score(x_test, y_test)
            y_pred = model.predict(x_test)
            st.write("Accuracy: ", accuracy.round(2))
